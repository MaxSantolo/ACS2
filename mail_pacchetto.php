...
$sql = "SELECT * FROM acs_pacchetti WHERE id_pacchetto = ".$id."" ;
$result = $conn->query($sql);

$sql_scadenza = "SELECT * FROM acs_pacchetti WHERE cestinato != '1' && (((data_fine_pacchetto - curdate())/(data_fine_pacchetto-data_inizio_pacchetto))< 0.2 || ore_utilizzate / (ore_totali_pacchetto + delta_ore) > 0.8) && id_pacchetto = ".$id."";
$resultscadenza = $conn->query($sql_scadenza); 
if ($resultscadenza->num_rows > 0) { $testo_messaggio = "Il suo contratto &egrave; <strong>in scadenza</strong>:<BR>"; }
else { $testo_messaggio = "Questi sono i dettagli del suo contratto:<BR>";}
...
$corpodeltesto = "<table style=\"margin: 0pt auto; border: 3px solid #015d6e; width: 640px;border-radius:20px;-moz-border-radius:20px;-webkit-border-radius:20px\">"
        ...
        . "     <TR><TD style=\"background:#cccccc;\"><strong>Ore totali: </strong></TD><TD><STRONG>".$row['ore_totali_pacchetto']."</STRONG></TD></TR>"
        ...
        . "     </TD></table> ";
$corpodeltestotxt = "Il messaggio � formattato in HTML, attivare tale modalit�.";
}
