...
class PickLog
{
    ...
    function write2File($app,$content,$action = '',$user = '') {

        $now = (new DateTime("Europe/Rome"))->format('d-m-Y H:i:s');
        $logfile = $_SERVER['DOCUMENT_ROOT']."/logs/" . $app. "/" . (new DateTime("Europe/Rome"))->format(Ymd) . "_" . $app . ".txt"; //genero il nome del file
        $text = "
START: {$now}>---------------------------------
APP: {$app}
AZN: {$action}
